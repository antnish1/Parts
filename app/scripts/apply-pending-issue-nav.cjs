...
source = source.replace(`desktopOrder: 12 },\n  { to: '/approvals/pending'`, `desktopOrder: 13 },\n  { to: '/approvals/pending'`);
replaceOnce(`  const managerApprovalCount = (adminCounterQuery.data ?? []).filter((order) => \`${'${order.status} ${order.approval_status}'}\`.toLowerCase().replace(/[^a-z]/g, '').includes('pendingmanagerapproval')).length;`, `  const managerApprovalCount = (adminCounterQuery.data ?? []).filter((order) => \`${'${order.status} ${order.approval_status}'}\`.toLowerCase().replace(/[^a-z]/g, '').includes('pendingmanagerapproval')).length;\n\n  const pendingIssueQuery = useQuery({\n    queryKey: ['pending-issue-nav-count', profile?.role, profile?.branch],\n    queryFn: getPendingIssueOrders,\n    enabled: ['branch', 'admin', 'manager', 'developer', 'hq'].includes(profile?.role ?? ''),\n    staleTime: 30000,\n    refetchOnWindowFocus: true,\n  });\n  const pendingIssueCount = pendingIssueQuery.data?.length ?? 0;`, 'pending issue query');
replaceOnce(`      if (isManager && item.to === '/approvals/pending') return { ...item, badge: managerApprovalCount };\n      return item;`, `      if (isManager && item.to === '/approvals/pending') return { ...item, badge: managerApprovalCount };\n      if (item.to === '/orders/pending-issue') return { ...item, badge: pendingIssueCount };\n      return item;`, 'pending issue badge');
replaceOnce(`    [approvedOrdersCount, isAdmin, isManager, managerApprovalCount],`, `    [approvedOrdersCount, isAdmin, isManager, managerApprovalCount, pendingIssueCount],`, 'pending issue memo deps');
