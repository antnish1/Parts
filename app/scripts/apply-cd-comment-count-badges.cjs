...
// Keep the comment count on the list record so both desktop and mobile can render it.
{
  ...
  if (!source.includes("portal_credit_dispatch_events').select('dispatch_id')")) {
...
// Render the count beside each dispatch ID. Existing row/card navigation remains unchanged.
{
  ...
  if (!source.includes('aria-label={`${row.comment_count} comments`}')) {
    ...
    source = source.replace(dispatchIdPattern, (_match, className) => {
      replacements += 1;
      return `<div className="flex min-w-0 items-center gap-2"><p className="${className}">{row.dispatch_no ?? 'Pending No.'}</p>{Number(row.comment_count ?? 0) > 0 ? <span aria-label={\`${'${row.comment_count}'} comments\`} title={\`${'${row.comment_count}'} comment${'${Number(row.comment_count) === 1 ? \'\' : \'s\'}'}\`} className="inline-flex h-5 min-w-5 shrink-0 items-center justify-center rounded-full bg-red-500 px-1.5 text-[10px] font-bold leading-none text-white">{row.comment_count}</span> : null}</div>`;
    });
