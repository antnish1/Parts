...
export type NavItem = { to: string; label: string; icon: LucideIcon; badge?: number; group?: string };
  ...
  return (
    <nav className="pc-nav mt-3 space-y-0.5">
      {visibleItems.map((item) => {
        ...
        const showGroup = !collapsed && !!item.group && item.group !== previousGroup;
        if (item.group) previousGroup = item.group;

        return (
          <Fragment key={item.to}>
            {showGroup ? <p className="pc-nav-group">{item.group}</p> : null}
            <NavLink
              to={item.to}
