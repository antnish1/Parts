    ...
    <PageCard eyebrow="Inventory" title="Inventory Lookup" description="Branch and item stock review workspace.">
      ...
      <div className="overflow-hidden rounded-lg border border-[#263244]">
        <table className="w-full min-w-[860px] border-collapse text-left text-xs">
          ...
          <tbody className="divide-y divide-[#263244] bg-[#111827]">{filteredRows.map((row) => (<tr key={row.id} className="hover:bg-slate-50"><td className="px-2.5 py-2 font-black text-white">{row.branch_code}</td><td className="px-2.5 py-2 text-[#d8e3ee]">{row.item_code}</td><td className="px-2.5 py-2 text-[#d8e3ee]">{row.item_name ?? '-'}</td><td className="px-2.5 py-2 text-[#d8e3ee]">{row.item_group ?? '-'}</td><td className="px-2.5 py-2 text-[#d8e3ee]">{row.uom ?? '-'}</td><td className="px-2.5 py-2 text-right text-[#d8e3ee]">{row.qty}</td><td className="px-2.5 py-2 text-right text-[#d8e3ee]">{row.dnp ?? '-'}</td><td className="px-2.5 py-2 text-right text-[#d8e3ee]">₹{Number(row.inv_value ?? 0).toLocaleString('en-IN', { maximumFractionDigits: 0 })}</td></tr>))}</tbody>
        </table>
