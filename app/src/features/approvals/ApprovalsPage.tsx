...
import { acceptTestOrderReviewEdits, approveTestOrderWithOriginalQty, resetTestOrderItemQty, setTestOrderApproved, setTestOrderManagerApproved, setTestOrderManagerRejected, setTestOrderRejected, updateTestOrderItemQty, zeroTestOrderItemForReview } from '../../services/testApproval.service';
...
export function ApprovalsPage() {
  ...
  const filteredOrders = useMemo(() => {
    ...
    return pendingOrders.filter((order) => `${order.order_no} ${order.branch} ${order.order_type} ${order.customer_name ?? ''} ${order.machine_no ?? ''} ${order.approver?.full_name ?? ''}`.toLowerCase().includes(term));
  ...
  const blockingLabel = busyId.includes('reject') ? 'Rejecting order' : busyId.includes('approve') || busyId.includes('Approve') ? 'Approving order' : busyId.includes('acceptEdits') ? 'Accepting edits' : busyId.includes('approveOriginal') ? 'Approving original qty' : 'Saving changes';
  ...
  return (
    <PageCard eyebrow="Approvals" title="Approval Queue" description="Review, approve, reject, or send orders to final manager approval.">
      ...
      <div className="mb-2 flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
        <input className="rounded-md border border-[#263244] bg-[#0b1020] px-2.5 py-1.5 text-xs text-white outline-none focus:border-[#82C8E5] lg:w-96" placeholder="Search order, branch, customer, machine, approver" value={search} onChange={(event) => setSearch(event.target.value)} disabled={isBlockingAction} />
