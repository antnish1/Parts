...
export function DocketScannerPage() {
  ...
  async function lookupOrders(valueOverride?: string) {
    ...
    setScannerStatus(value ? `Searching item rows for ${value}...` : 'Enter docket, invoice, order or machine number.');
    if (!value) return;
    try {
      const rows = await lookupTestDocketOrders(value);
      setOrders(rows);
      setScannerStatus(rows.length ? `${rows.length} matching order(s) found. Receive action will update only matching item rows.` : 'No matching item rows/order found.');
    } catch (error) {
    ...
    try {
      ...
      scanTimerRef.current = window.setInterval(() => {
        ...
        detector.detect(video).then((codes) => {
          ...
          setDocketNo(raw);
          setSearchTerm('');
          setScannerStatus(`Detected ${raw}. Searching matching item rows...`);
  ...
  async function receiveOrder(order: TestDocketOrder) {
    ...
    setBusyId(order.id);
    setScannerStatus('Updating matching item rows as received...');
    try {
      await markTestDocketReceived(order, value);
      setScannerStatus(`${order.final_order_no || order.order_no}: matching item rows marked received.`);
  ...
  return (
    <PageCard eyebrow="Docket" title="Docket Scanner" description="Scan or enter docket/invoice details and mark matching item rows received.">
      <div className="grid gap-3 xl:grid-cols-[0.75fr_1.25fr]">
        ...
        <div className="rounded-lg border border-[#263244] bg-[#0b1020] p-3">
          <p className="mb-2 text-xs font-black uppercase tracking-[0.12em] text-[#82C8E5]">Manual Item Receive</p>
          <div className="grid gap-2 lg:grid-cols-[1fr_1fr_auto]">
            <input className="rounded-md border border-[#263244] bg-[#111827] px-2.5 py-1.5 text-xs text-white outline-none focus:border-[#82C8E5]" value={docketNo} onChange={(event) => setDocketNo(normalizeDocketNo(event.target.value))} placeholder="Docket / invoice number" />
            <input className="rounded-md border border-[#263244] bg-[#111827] px-2.5 py-1.5 text-xs text-white outline-none focus:border-[#82C8E5]" value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} placeholder="Order / machine" />
