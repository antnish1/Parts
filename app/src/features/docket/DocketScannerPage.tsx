                  ...
                  return (
                    <Fragment key={key}>
                      <tr className={getStatusRowClasses(row.item_status || row.order_status)}>
                        ...
                        <td className="px-2 py-2 font-black text-white">{row.docket_no || '-'}</td><td className="px-2 py-2 text-[#d8e3ee]">{row.invoice_no || '-'}</td><td className="px-2 py-2 font-black text-[#82C8E5]">{row.part_no}</td><td className="px-2 py-2 text-[#d8e3ee]">{row.description || '-'}</td><td className="px-2 py-2 text-right font-black text-white">{row.billed_qty}</td><td className="px-2 py-2 text-right text-[#d8e3ee]">{row.received_qty}</td><td className="px-2 py-2 text-[#d8e3ee]">{row.final_order_no || row.order_no}</td><td className="px-2 py-2 text-[#d8e3ee]">{row.branch}</td><td className="px-2 py-2"><StatusBadge status={row.item_status || row.order_status} /></td><td className="px-2 py-2 text-right"><button className="font-black text-[#82C8E5] hover:underline disabled:text-[#6D8196] disabled:no-underline" disabled={busyId === key || isReceived} onClick={() => void receiveRow(row)}>{busyId === key ? 'Saving' : isReceived ? 'Received' : 'Receive'}</button></td>
                      </tr>
                      {isExpanded ? <tr><td colSpan={11} className="bg-[#0b1020] px-4 py-3"><div className="grid gap-2 text-xs md:grid-cols-2 xl:grid-cols-4"><div className="rounded-md border border-[#263244] bg-[#111827] p-2"><p className="text-[10px] uppercase text-[#6D8196]">Order No</p><p className="font-black text-white">{row.order_no}</p></div><div className="rounded-md border border-[#263244] bg-[#111827] p-2"><p className="text-[10px] uppercase text-[#6D8196]">Customer / Machine</p><p className="font-black text-white">{row.customer_name || '-'} / {row.machine_no || '-'}</p></div><div className="rounded-md border border-[#263244] bg-[#111827] p-2"><p className="text-[10px] uppercase text-[#6D8196]">Qty</p><p className="font-black text-white">Ordered {effectiveQty(row)} • Billed {row.billed_qty} • Received {row.received_qty}</p></div><div className="rounded-md border border-[#263244] bg-[#111827] p-2"><p className="text-[10px] uppercase text-[#6D8196]">Transport</p><p className="font-black text-white">{row.transport_name || '-'} • {row.delivery_no || '-'}</p></div><div className="rounded-md border border-[#263244] bg-[#111827] p-2"><p className="text-[10px] uppercase text-[#6D8196]">Billing Date</p><p className="font-black text-white">{row.billing_date || '-'}</p></div><div className="rounded-md border border-[#263244] bg-[#111827] p-2"><p className="text-[10px] uppercase text-[#6D8196]">Received At</p><p className="font-black text-white">{formatDate(row.received_at)}</p></div><div className="rounded-md border border-[#263244] bg-[#111827] p-2"><p className="text-[10px] uppercase text-[#6D8196]">Order Status</p><StatusBadge status={row.order_status} /></div><div className="rounded-md border border-[#263244] bg-[#111827] p-2"><p className="text-[10px] uppercase text-[#6D8196]">Open Order</p><Link className="font-black text-[#82C8E5] hover:underline" to={`/orders/${row.order_id}`}>Open full order detail</Link></div></div></td></tr> : null}
                    </Fragment>
                  );
                })}
              </tbody>
