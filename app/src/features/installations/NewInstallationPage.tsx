...
export function NewInstallationPage() {
  ...
  return <PageCard eyebrow="Installation Management" title="Add New Engine & Breaker Entry" description="Create one installation entry manually or upload multiple entries from Excel">
    ...
    <ConfirmDialog open={confirmOpen} title="Create installation entry?" description={`Create this ${equipmentType === 'ENGINE' ? 'Engine' : 'Rock Breaker'} entry for ${branch || 'the selected branch'}?`} confirmLabel="Create Entry" pending={mutation.isPending} onCancel={() => setConfirmOpen(false)} onConfirm={confirmCreate} />
  </PageCard>;
}
