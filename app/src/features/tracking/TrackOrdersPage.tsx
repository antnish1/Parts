...
export function TrackOrdersPage() {
  ...
  const filteredOrders = useMemo(() => {
    ...
    const filtered = orders.filter((order) => {
      ...
      const matchesSearch = !term || `${order.order_no} ${order.final_order_no ?? ''} ${order.branch} ${getOrderForLabel(order)} ${order.customer_name ?? ''} ${order.machine_no ?? ''} ${order.order_type} ${order.status} ${order.dbms_invoice_no ?? ''}`.toLowerCase().includes(term);
