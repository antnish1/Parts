...
function escapeCsv(value: string | number | null | undefined) { const text = String(value ?? ''); return `"${text.replace(/"/g, '""')}"`; }
...
export function ManagerDashboardPage() {
  ...
  const txnQuery = useQuery({ queryKey: ['manager-inventory-txn', inventoryBranch, effectiveInventoryDate], queryFn: () => getManagerInventoryTransactions(inventoryBranch, effectiveInventoryDate) });
  ...
  const inventoryBranches = useMemo(() => [...new Set((inventoryQuery.data ?? []).map((row) => row.branch_code))].sort(), [inventoryQuery.data]);
  ...
  const totals = filteredOrders.reduce((acc, order) => { const meta = metaMap[order.id] ?? { totalQty: 0, totalValue: 0, commentCount: 0 }; acc.qty += meta.totalQty; acc.value += meta.totalValue; acc.comments += meta.commentCount; return acc; }, { qty: 0, value: 0, comments: 0 });
  const data = {
  ...
  function exportOrders() {
    downloadCsv(`manager-orders-${new Date().toISOString().slice(0, 10)}.csv`, ['Order No', 'Final No', 'Branch', 'Customer', 'Machine', 'Type', 'For', 'Status', 'Qty', 'Value', 'Comments', 'Created'], filteredOrders.map((order) => { const meta = metaMap[order.id] ?? { totalQty: 0, totalValue: 0, commentCount: 0 }; return [order.order_no, order.final_order_no ?? '', order.branch, order.customer_name ?? '', order.machine_no ?? '', order.order_type, order.order_for, order.status, meta.totalQty, meta.totalValue, meta.commentCount, order.created_at.slice(0, 10)]; }));
  }
  function exportInventory() {
    downloadCsv(`manager-inventory-${effectiveInventoryDate || 'latest'}.csv`, ['Report Date', 'Branch Code', 'Branch Name', 'Part No', 'Item Name', 'Group', 'UOM', 'Qty', 'DNP', 'Value'], inventoryRows.map((row) => [row.report_date, row.branch_code, row.branch_name, row.item_code, row.item_name, row.item_group, row.uom, row.qty, row.dnp, row.inv_value]));
  }
  function exportTransactions() {
    downloadCsv(`manager-inventory-transactions-${effectiveInventoryDate || 'latest'}.csv`, ['Report Date', 'Branch Code', 'Branch Name', 'Part No', 'Item Name', 'Group', 'Received', 'Issued', 'Closing Balance', 'Value'], txnRows.map((row) => [row.report_date, row.branch_code, row.branch_name, row.item_code, row.item_name, row.item_group, row.received, row.issued, row.closing_balance, row.closing_value]));
  }
  ...
  return (
    <PageCard eyebrow="Manager" title="Manager Dashboard" description="Operational order and inventory summary workspace.">
      ...
      <div className="mb-3 grid gap-2 lg:grid-cols-[1fr_1fr_140px_140px_auto]">
        <select className="rounded-md border border-[#263244] bg-[#0b1020] px-2.5 py-1.5 text-xs text-white outline-none focus:border-[#82C8E5]" value={branchFilter} onChange={(event) => setBranchFilter(event.target.value)}><option value="all">All Branches</option>{orderBranches.map((branch) => <option key={branch} value={branch}>{branch}</option>)}</select>
