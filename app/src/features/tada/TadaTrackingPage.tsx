...
import { CheckCircle2, CircleAlert, FilePlus2, Landmark, Search, WalletCards } from 'lucide-react';
    ...
    <PageCard eyebrow="Operations" title="TA/DA Bill Tracking" description="Track SVR movement from branch dispatch through HQ and Accounts receipt.">
      ...
      <div className="mt-2 space-y-2 md:hidden">
        {visible.map((item) => {
          ...
          return <button key={item.id} type="button" onClick={() => navigate(`/ta-da/${item.id}`)} className={`relative w-full overflow-hidden rounded-lg border p-2.5 text-left shadow-[0_1px_2px_rgba(15,23,42,0.04)] ${meta.surfaceClass}`}>
            <span className={`absolute inset-y-0 left-0 w-1 ${meta.accentClass}`} />
