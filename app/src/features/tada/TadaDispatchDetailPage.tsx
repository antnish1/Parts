...
export function TadaDispatchDetailPage() {
  ...
  if (isLoading) return <PageCard eyebrow="TA/DA Bills" title="Loading Dispatch"><p className="text-xs font-bold text-[#64748b]">Loading TA/DA movement details…</p></PageCard>;
  if (error || !dispatch) return <PageCard eyebrow="TA/DA Bills" title="Dispatch Not Available"><p className="text-xs font-bold text-[#b91c1c]">{error instanceof Error ? error.message : 'Unable to load dispatch.'}</p></PageCard>;
  ...
  return (
    <PageCard eyebrow="TA/DA Bills" title={dispatch.dispatch_no} description={`${dispatch.branch_name_snapshot} • ${statusLabels[dispatch.status] ?? dispatch.status}`}>
      ...
      <section className="mt-3 rounded-lg border border-[#dbe3ec] bg-white p-3"><p className="mb-2 text-xs font-black text-[#172033]">Traceability Timeline</p><div className="space-y-2">{(data?.events ?? []).map((event) => <div key={event.id} className="border-l-2 border-[#bfdbfe] pl-3"><p className="text-xs font-black text-[#172033]">{event.event_type.replaceAll('_', ' ')}</p><p className="text-[11px] text-[#64748b]">{new Date(event.created_at).toLocaleString()} • {event.actor_name_snapshot ?? 'System'}{event.actor_role_snapshot ? ` (${event.actor_role_snapshot})` : ''}</p></div>)}{!data?.events.length ? <p className="text-xs text-[#64748b]">No timeline events yet.</p> : null}</div></section>
    </PageCard>
  );
}
