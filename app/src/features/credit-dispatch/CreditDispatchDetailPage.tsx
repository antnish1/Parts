...
function Box({ label, value }: { label: string; value: string }) {
  return <div className="rounded-2xl bg-slate-50 p-3 print:border print:border-slate-200"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-400">{label}</p><p className="mt-1 text-sm font-black text-slate-900">{value || '-'}</p></div>;
}

function SignatureBox({ label, url }: { label: string; url: string | null }) {
  return <div className="rounded-2xl border border-slate-200 p-3"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-400">{label}</p>{url ? <img src={url} alt={label} className="mt-2 h-24 w-full object-contain" /> : <div className="mt-2 grid h-24 place-items-center rounded-xl bg-slate-50 text-xs font-bold text-slate-400">Signature not available</div>}</div>;
}
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-4 pb-20 print:pb-0">
      ...
      <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm print:border-none print:p-0 print:shadow-none">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-3"><Box label="Branch" value={dispatch.branch} /><Box label="Customer" value={dispatch.customer_name} /><Box label="Mobile" value={dispatch.mobile_no} /><Box label="Customer Type" value={dispatch.customer_type} /><Box label="Document" value={`${dispatch.document_type} ${dispatch.document_no || ''}`} /><Box label="Document Date" value={dispatch.document_date} /><Box label="Closure" value={`Within ${dispatch.tentative_closure_days} Days`} /><Box label="Due Date" value={dispatch.due_date} /><Box label="Status" value={`${dispatch.approval_status} / ${dispatch.recovery_status}`} /></div>
        ...
        {dispatch.remarks ? <div className="mt-4 rounded-2xl bg-slate-50 p-3 print:border print:border-slate-200"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-400">Remarks</p><p className="mt-1 text-sm font-bold text-slate-700">{dispatch.remarks}</p></div> : null}
        <div className="mt-4 grid gap-3 md:grid-cols-2"><SignatureBox label="Customer Signature" url={customerSignatureUrl} /><SignatureBox label="Issuing Official Signature" url={issuerSignatureUrl} /></div>
        <div className="mt-4 grid gap-3 md:grid-cols-2"><Box label="Created At" value={formatDateTime(dispatch.created_at)} /><Box label="Approved At" value={dispatch.approved_at ? formatDateTime(dispatch.approved_at) : '-'} /></div>
      </section>
