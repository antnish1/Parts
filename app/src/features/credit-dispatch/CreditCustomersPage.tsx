...
function CustomerCard({ row }: { row: CreditCustomerOutstanding }) {
  return (
    <article className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <Link to={'/credit-dispatch/customers/ledger?id=' + row.customer_id} className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-blue-200 bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-700"><BookOpen className="h-4 w-4" />View Ledger</Link>
    </article>
  );
}
...
export function CreditCustomersPage() {
  ...
  return (
    <div data-cd-theme="customers" className="cd-shell mx-auto max-w-7xl space-y-4 pb-20">
      <div className="cd-hero rounded-3xl border border-slate-200 bg-white p-4 shadow-sm"><Link to="/credit-dispatch" className="mb-2 inline-flex items-center gap-2 text-sm font-medium text-slate-500"><ArrowLeft className="h-4 w-4" />Back</Link><p className="text-[11px] font-medium uppercase tracking-[0.15em] text-blue-600">Customer Outstanding</p></div>
