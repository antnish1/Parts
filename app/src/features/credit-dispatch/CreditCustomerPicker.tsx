...
export function CreditCustomerPicker({ value, onChange, onSelect }: { value: string; onChange: (value: string) => void; onSelect: (customer: CustomerSuggestion) => void }) {
  ...
  useEffect(() => {
    if (!hasSearchTerm) setOpen(false);
  }, [hasSearchTerm]);
    ...
    <div ref={rootRef} className="relative">
      <label className="block">
        ...
        <div className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-3 focus-within:border-blue-400 focus-within:ring-4 focus-within:ring-blue-100">
          ...
          <input
            className="min-w-0 flex-1 bg-transparent text-base font-medium text-slate-900 outline-none placeholder:text-slate-300"
            value={value}
      ...
      {open && hasSearchTerm && (query.isLoading || suggestions.length > 0) ? (
        <div className="absolute z-50 mt-2 max-h-80 w-full overflow-auto rounded-3xl border border-slate-200 bg-white p-2 shadow-2xl">
          {query.isLoading ? <p className="p-3 text-sm font-medium text-slate-500">Searching...</p> : suggestions.map((customer) => (
      ...
      {open && hasSearchTerm && !query.isLoading && suggestions.length === 0 ? (
        <p className="mt-2 rounded-2xl border border-blue-100 bg-blue-50 px-3 py-2 text-xs font-medium text-blue-700">
          No matching customer. This customer will be created after submission.
        </p>
      ) : null}
