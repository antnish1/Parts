    ...
    <div className="mx-auto max-w-4xl pb-24">
        ...
        {step === 3 ? (
          <section className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              ].map(([label, value]) => <div key={label} className="rounded-2xl bg-slate-50 p-3"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-400">{label}</p><p className="mt-1 break-words text-sm font-black text-slate-900">{value}</p></div>)}
            </div>
            {createMutation.error ? <div className="mt-4 rounded-2xl border border-red-200 bg-red-50 p-3 text-sm font-bold text-red-700">{(createMutation.error as Error).message || 'Could not submit credit dispatch request.'}</div> : null}
          </section>
        ) : null}
      </div>
      ...
      <div className="fixed inset-x-0 bottom-0 z-30 border-t border-slate-200 bg-white/95 px-3 py-3 shadow-2xl backdrop-blur lg:left-48">
        <div className="mx-auto flex max-w-4xl items-center justify-between gap-3">
          <Button type="button" variant="secondary" className="px-3" disabled={step === 0 || createMutation.isPending} onClick={() => setStep((current) => Math.max(current - 1, 0))}><ArrowLeft className="h-4 w-4" />Back</Button>
          <div className="hidden items-center gap-2 text-xs font-bold text-slate-500 sm:flex"><Save className="h-4 w-4" />Draft auto-saved on this device</div>
          {step < 3 ? <Button type="button" onClick={goNext} disabled={createMutation.isPending}>Next<ArrowRight className="h-4 w-4" /></Button> : <Button type="button" onClick={submit} disabled={createMutation.isPending}><Send className="h-4 w-4" />{createMutation.isPending ? 'Submitting...' : 'Submit'}</Button>}
        </div>
      </div>
    </div>
  );
