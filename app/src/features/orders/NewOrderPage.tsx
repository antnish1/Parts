...
export function NewOrderPage() {
  ...
  function partCategory(partNo: string) {
    const part = partCache[normalizePartNo(partNo)];
    return part?.cat1 || part?.cat2 || '-';
  }
  ...
  async function refreshPreviousQty(lineId: number, partNo: string, branch = form.branch) {
    ...
    try {
      updateItem(lineId, 'previous30dQty', await getTestLast30QtyByBranchPart(branch, normalized, 30));
    } catch (error) {
  ...
  function handlePartChange(lineId: number, value: string) {
    ...
    setItems((current) => current.map((item) => (item.lineId === lineId ? { ...item, partNo: normalized, description: '', dnp: '', previous30dQty: 0 } : item)));
  }

  async function handlePartLookup(lineId: number, value: string) {
    const normalized = normalizePartNo(value);
    if (!normalized) return;

    const cachedPart = partCache[normalized];
    if (cachedPart) {
      setItems((current) => current.map((item) => (item.lineId === lineId && normalizePartNo(item.partNo) === normalized ? { ...item, description: cachedPart.description ?? '', dnp: cachedPart.dnp != null ? String(cachedPart.dnp) : '' } : item)));
    ...
    try {
      ...
      if (!part) {
        ...
        setMessage(`Part not found in part_master: ${normalized}`);
        return;
      }

      setPartCache((current) => ({ ...current, [part.part_no]: part }));
  ...
  async function handleBulkUpload(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const result = await parseBulkPartsFile(file, bulkHasHeader);
      const nextItems: ItemLine[] = result.rows.map((row, index) => ({ lineId: Date.now() + index, partNo: row.partNo, description: row.description, dnp: String(row.dnp), qty: String(row.qty), previous30dQty: 0 }));
      ...
      setItems(nextItems);
      setPartCache((current) => {
        const next = { ...current };
        result.rows.forEach((row) => {
          if (!row.isUnknown) next[row.partNo] = { part_no: row.partNo, description: row.description, dnp: Number(row.dnp), cat1: null, cat2: null };
        });
        return next;
      });
  ...
  return (
    <PageCard eyebrow="Orders" title="New Order" description="Create branch order.">
          ...
          <section className="rounded-2xl border border-[#334155] bg-[#0b1020] p-5">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div>
                ...
                <select className={inputClass} value={form.orderFor} disabled={isFormBusy || form.orderType === 'VOR'} onChange={(e) => updateField('orderFor', e.target.value)}>
              ...
              <div>
                ...
                <input className={inputClass} value={profile?.fullName ?? ''} placeholder="Employee Name" readOnly />
              </div>
              <div>
                <label className={labelClass}>Approved By</label>
                <select className={inputClass} value={form.approverId} onChange={(e) => updateField('approverId', e.target.value)} disabled={isFormBusy}>
                  <option value="">Select Approver</option>
                  {approvers.map((approver) => <option key={approver.id} value={approver.id}>{approver.full_name} ({approver.role})</option>)}
                </select>
              </div>
            </div>
          </section>
      ...
      {machineLookupLoading ? (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-[#020617]/70 px-4 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-3xl border border-[#82C8E5]/40 bg-[#0b1020] p-6 text-center shadow-[0_0_80px_rgba(130,200,229,0.24)]">
            <ActionLoader variant="scanner" label="Searching customer" />
            <p className="mt-4 text-xs font-semibold leading-5 text-[#c7d2df]">Please wait. The screen is locked while machine_master is being searched.</p>
          </div>
        </div>
      ) : null}
