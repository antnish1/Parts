...
export function NewOrderPage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (order) => { const approver = approvers.find((item) => item.id === form.approverId); setOrderSummary({ orderNo: order.order_no, branch: form.branch, orderType: form.orderType, orderFor: form.orderFor, customerName: form.customerName, approverName: approver?.full_name ?? '', totalItems: items.length, totalValue }); setMessage('Order created successfully.'); queryClient.invalidateQueries({ queryKey: ['test-orders'] }); queryClient.invalidateQueries({ queryKey: ['test-dashboard-summary'] }); queryClient.invalidateQueries({ queryKey: ['order-list-paged'] }); },
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setMessage(''); setOrderSummary(null); const parsedItems = items.map((item) => ({ partNo: normalizePartNo(item.partNo), description: item.description.trim(), dnp: Number(item.dnp), qty: Number(item.qty), previous30dQty: item.previous30dQty || 0 })); const duplicatePart = parsedItems.find((item, index) => parsedItems.findIndex((candidate) => candidate.partNo === item.partNo) !== index); const invalidItem = parsedItems.find((item) => !item.partNo || !item.description || !Number.isFinite(item.dnp) || !Number.isFinite(item.qty) || item.dnp < 0 || item.qty <= 0 || !Number.isInteger(item.qty)); if (!form.branch || !form.orderType || !form.orderFor) return setMessage('Please fill Order Type and Order For.'); if (!form.approverId) return setMessage('Please select approver.'); if (form.orderType === 'VOR' && form.orderFor !== 'Customer') return setMessage('VOR order must be for Customer.'); if (form.orderFor === 'Customer' && (!form.machineNo || !form.customerName || !form.warrantyStatus)) return setMessage('Customer order requires machine number, customer name, and machine type.'); if (duplicatePart) return setMessage(`Duplicate item not allowed: ${duplicatePart.partNo}`); if (invalidItem) return setMessage('Each item must have valid master part, Qty, Description, and DNP. Qty must be a whole number above zero.'); mutation.mutate({ branch: form.branch, orderType: form.orderType, orderFor: form.orderFor, approverId: form.approverId, machineNo: form.orderFor === 'Stock' ? '' : normalizeMachineNo(form.machineNo), customerName: form.orderFor === 'Stock' ? '' : form.customerName, callId: form.callId, warrantyStatus: form.orderFor === 'Stock' ? 'NA' : form.warrantyStatus, items: parsedItems }); }
