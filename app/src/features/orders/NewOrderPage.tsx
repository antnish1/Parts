...
const inputClass = 'mt-2 w-full rounded-md border border-[#263244] bg-[#0b1020] px-2.5 py-1.5 text-xs text-white outline-none focus:border-[#82C8E5]';
...
export function NewOrderPage() {
  ...
  const [form, setForm] = useState({ branch: 'Jabalpur BHL', orderType: 'VOR', orderFor: 'Customer', machineNo: 'JCB3DX-TEST', customerName: 'Demo Customer', callId: 'CALL-TEST', warrantyStatus: 'Warranty' });
  ...
  function updateItem(lineId: number, field: keyof ItemLine, value: string | number) {
  ...
  async function refreshPreviousQty(lineId: number, partNo: string, branch = form.branch) {
    ...
    try {
      const qty = await getTestLast30QtyByBranchPart(branch, normalized, 30);
      updateItem(lineId, 'previous30dQty', qty);
    } catch (error) {
      console.warn('Previous quantity lookup failed', error);
    }
  }
  ...
  function addItem() {
    const next = { ...defaultItem, lineId: Date.now(), previous30dQty: 0 };
    setItems((current) => [...current, next]);
    void refreshPreviousQty(next.lineId, next.partNo);
  }
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (form.orderType === 'VOR' && (!form.machineNo || !form.customerName || !form.warrantyStatus)) return setMessage('VOR order requires machine, customer, and machine type/status.');
    if (form.orderFor === 'Customer' && (!form.machineNo || !form.customerName)) return setMessage('Customer order requires machine and customer details.');
    ...
    mutation.mutate({ branch: form.branch, orderType: form.orderType, orderFor: form.orderFor, machineNo: form.machineNo, customerName: form.customerName, callId: form.callId, warrantyStatus: form.warrantyStatus, items: parsedItems });
  }
    ...
    <PageCard eyebrow="Orders" title="New Order" description="Create multi-item branch order.">
      <form onSubmit={handleSubmit} className="space-y-3">
        <div className="grid gap-3 lg:grid-cols-4">
          ...
          <div><label className={labelClass}>Machine Type</label><select className={inputClass} value={form.warrantyStatus} onChange={(e) => updateField('warrantyStatus', e.target.value)}><option>UW</option><option>BW</option><option>NA</option></select></div>
          ...
          <div className="rounded-md border border-[#263244] bg-[#0b1020] px-2.5 py-2"><p className={labelClass}>Order Value</p><p className="mt-1 text-lg font-black text-white">₹{totalValue.toFixed(2)}</p></div>
        </div>
        ...
        <div className="rounded-lg border border-[#263244] bg-[#0b1020] p-3">
          ...
          <div className="space-y-2">
            {items.map((item, index) => (<div key={item.lineId} className="grid gap-2 rounded-md border border-[#263244] p-2 lg:grid-cols-8">
              <div><label className={labelClass}>Lookup</label><select className={inputClass} value={item.partNo} onChange={(e) => selectPart(item.lineId, e.target.value)}>{parts.map((part) => <option key={part.part_no} value={part.part_no}>{part.part_no}</option>)}</select></div>
              <div><label className={labelClass}>Part No</label><input className={inputClass} value={item.partNo} onBlur={() => void refreshPreviousQty(item.lineId, item.partNo)} onChange={(e) => updateItem(item.lineId, 'partNo', e.target.value)} /></div>
              <div className="lg:col-span-2"><label className={labelClass}>Description</label><input className={inputClass} value={item.description} onChange={(e) => updateItem(item.lineId, 'description', e.target.value)} /></div>
              <div><label className={labelClass}>30D Qty</label><input className={inputClass} value={item.previous30dQty} readOnly /></div>
              <div><label className={labelClass}>DNP</label><input className={inputClass} value={item.dnp} onChange={(e) => updateItem(item.lineId, 'dnp', e.target.value)} /></div>
              <div><label className={labelClass}>Qty</label><input className={inputClass} value={item.qty} onChange={(e) => updateItem(item.lineId, 'qty', e.target.value)} /></div>
              <div className="flex items-end justify-end"><Button type="button" variant="danger" className="rounded-md px-3 py-1.5 text-xs" onClick={() => removeItem(item.lineId)}>Remove</Button></div>
              <div className="lg:col-span-8 text-xs text-[#c7d2df]">Line {index + 1}: ₹{(Number(item.dnp || 0) * Number(item.qty || 0)).toFixed(2)} • Previous 30D branch qty: {item.previous30dQty}</div>
            </div>))}
          </div>
        </div>
