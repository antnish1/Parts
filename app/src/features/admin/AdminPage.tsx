...
export function AdminPage() {
  ...
  async function processOrder(order: (typeof orders)[number]) {
    ...
    setBusyId(order.id);
    try {
      await setTestOrderProcessed(order);
      setMessage(`${order.order_no} processed.`);
      await refetch();
    } catch (error) {
  ...
  return (
    <PageCard eyebrow="Admin" title="Admin Processing" description="Process approved orders and review recent processed records.">
      ...
      <div className="grid gap-3 xl:grid-cols-[1.4fr_0.8fr]">
        <div className="overflow-hidden rounded-lg border border-[#263244]">
          <table className="w-full min-w-[840px] border-collapse text-left text-xs">
            ...
            <tbody className="divide-y divide-[#263244] bg-[#111827]">
              {approvedOrders.map((order) => (<tr key={order.id} className="hover:bg-[#182235]"><td className="px-2.5 py-2 font-black text-white">{order.order_no}</td><td className="px-2.5 py-2 text-[#d8e3ee]">{order.branch}</td><td className="px-2.5 py-2 text-[#d8e3ee]">{order.order_type}</td><td className="px-2.5 py-2 text-[#d8e3ee]">{order.customer_name ?? '-'}</td><td className="px-2.5 py-2 text-[#d8e3ee]">{order.machine_no ?? '-'}</td><td className="px-2.5 py-2"><StatusBadge status={order.status} /></td><td className="px-2.5 py-2 text-right"><div className="flex justify-end gap-3"><Link className="font-black text-[#82C8E5] hover:underline" to={`/orders/${order.id}`}>View</Link><button className="font-black text-[#82C8E5] hover:underline disabled:opacity-40" disabled={busyId === order.id} onClick={() => void processOrder(order)}>{busyId === order.id ? 'Processing' : 'Process'}</button></div></td></tr>))}
            </tbody>
          </table>
        ...
        <div className="rounded-lg border border-[#263244] bg-[#0b1020] p-3">
          ...
          <div className="space-y-2">
            {processedOrders.map((order) => (<div key={order.id} className="rounded-md border border-[#263244] bg-[#111827] px-2.5 py-2"><div className="flex items-center justify-between gap-2"><div><p className="text-xs font-black text-white">{order.order_no}</p><p className="text-xs text-[#c7d2df]">{order.branch} • {order.customer_name ?? '-'}</p></div><Link className="text-xs font-black text-[#82C8E5] hover:underline" to={`/orders/${order.id}`}>View</Link></div></div>))}
