  ...
  return (
    <div data-status-effects>
      <FeedbackModal
        message={message}
        onClose={() => {
          if (message) closedRef.current.add(message);
          setMessage('');
        }}
      />
      {loader && !message ? (
        <div className="fixed bottom-5 right-5 z-[90] rounded-2xl border border-[#263244] bg-[#020617]/90 p-4 shadow-[0_0_40px_rgba(56,189,248,0.18)] backdrop-blur-md">
          <ActionLoader variant={loader.variant} label={loader.label} />
        </div>
      ) : null}
    </div>
  );
