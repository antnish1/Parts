...
export function ConfirmDialog({ open, title, message, confirmLabel = 'Confirm', cancelLabel = 'Cancel', busy = false, tone = 'primary', onConfirm, onCancel }: { open: boolean; title: string; message: string; confirmLabel?: string; cancelLabel?: string; busy?: boolean; tone?: 'primary' | 'success'; onConfirm: () => void; onCancel: () => void }) {
  if (!open) return null;
  ...
  return <div className="fixed inset-0 z-[100] grid place-items-center bg-[#07111f]/55 p-4 backdrop-blur-[2px]" role="dialog" aria-modal="true" aria-labelledby="confirm-dialog-title">
    <div className="w-full max-w-[390px] overflow-hidden rounded-2xl border border-white/20 bg-white shadow-2xl">
      <div className="flex items-start gap-3 border-b border-[#e2e8f0] bg-[#f8fbff] p-4">
        <div className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-[#dceeff] text-[#0f5fa8]"><Icon className="h-5 w-5"/></div>
      ...
      <div className="flex justify-end gap-2 p-3">
        ...
        <button type="button" onClick={onConfirm} disabled={busy} className={`h-9 rounded-lg px-5 text-xs font-semibold text-white shadow-sm disabled:opacity-60 ${tone === 'success' ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-[#0f5fa8] hover:bg-[#0b4d8a]'}`}>{busy ? 'Please wait…' : confirmLabel}</button>
      </div>
    </div>
  </div>;
}
