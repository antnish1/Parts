...
export function getStatusRowClass(status: string) {
  ...
  if (normalized === 'ISSUED' || normalized === 'PARTIALLY DISPATCHED') return 'transition-colors bg-purple-50/70 hover:bg-purple-100/80';
  if (normalized === 'RECEIVED' || normalized === 'PARTIALLY RECEIVED') return 'transition-colors bg-cyan-50/70 hover:bg-cyan-100/80';
  if (normalized === 'REJECTED') return 'transition-colors bg-rose-50/70 hover:bg-rose-100/80';
...
export function getStatusLeftBorderClass(status: string) {
  ...
  if (normalized === 'ISSUED' || normalized === 'PARTIALLY DISPATCHED') return 'border-l-2 border-l-[#6d28d9]/55';
  if (normalized === 'RECEIVED' || normalized === 'PARTIALLY RECEIVED') return 'border-l-2 border-l-[#0f766e]/55';
  if (normalized === 'REJECTED') return 'border-l-2 border-l-[#be123c]/55';
